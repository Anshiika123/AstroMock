...
passages (book_retriever) -> one prompt -> get_interpretation()
(llm_provider — Anthropic or Gemini, whichever is configured).
...
# Mirrored in astromock-mcp's tools/horoscope_tool.py (horoscope-guidance
# MCP prompt); keep the two in sync.
# ---------------------------------------------------------------------------
...
def generate_guidance(kundali_data: dict, timeframe: str,
    ...
    """Guidance reading text (calls the configured LLM).
...
def generate_horoscope(kundali_data: dict, timeframe: str) -> str:
    ...
    Args:
        kundali_data: output of generate_kundali() (needs rashi/Moon sign;
            birth time may be unknown — transits are Moon-sign based).
...
# ---------------------------------------------------------------------------
# Test — all three timeframes on the verified Saharanpur chart
# ---------------------------------------------------------------------------

if __name__ == "__main__":
    from kundali_calculator import generate_kundali

    # Saharanpur, 2004-09-21 10:30 IST — Scorpio Moon (verified chart).
    chart = generate_kundali("2004-09-21", "10:30",
                             29.9680, 77.5459, "Asia/Kolkata")
    moon_sign = chart["rashi"]["sign"]
    print(f"Moon sign (rashi): {moon_sign}")
    assert moon_sign == "Scorpio", moon_sign

    # bad timeframe -> clear error
    try:
        generate_horoscope(chart, "1year")
    ...
    transits = get_current_transits()
    timeframes = ("today", "2weeks", "6months")
    for i, timeframe in enumerate(timeframes):
        print("=" * 70)
        print(f"TIMEFRAME: {timeframe}")
        print("-" * 70)
        print("transit data sent:")
        print(build_transit_data(moon_sign, transits, timeframe))
        print("-" * 70)
        try:
            print(generate_horoscope(chart, timeframe))
        except Exception as e:
            print(f"SKIPPED LLM call: {str(e)[:200]}")
        if i < len(timeframes) - 1:
            time.sleep(20)  # free-tier Gemini allows ~5 requests/minute
