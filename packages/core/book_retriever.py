...
Two parts:
1. One-time setup: `python book_retriever.py path/to/book.pdf` extracts the
   PDF paragraph-by-paragraph into book_index.json (next to this module) so
   the PDF is never re-parsed at request time. Requires: pip install
   pdfplumber (only if pdftotext is unavailable).
...
def load_book_index(path: Path = BOOK_INDEX_PATH) -> list[dict]:
    ...
    if not path.exists():
        raise FileNotFoundError(
            f"{path.name} not found. One-time setup required: run "
            "`python book_retriever.py <path-to-book.pdf>` to build the index."
        )
...
if __name__ == "__main__":
    ...
    # ---- Tests with a synthetic fixture ----
    fixture = {
        "source": "fixture",
        "paragraphs": [
    ...
    assert get_relevant_book_context(8, [], "Mars", 8,
                                     index_path=FIXTURE_PATH) == ""
    assert len(ctx.split()) <= MAX_CONTEXT_WORDS + 60
